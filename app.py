...
import telebot
from telebot import *
from hidden_variable import API_KEY,URL,  _update_
import time
...
bot.remove_webhook()
bot.set_webhook(url=URL+"/webhook",max_connections=100)
logger = log().logger
user_data = {}
quiz_state = {}
active_group = {}
    ...
    try:
        ...
        if (len(msg) == 16) and msg.isupper():
            ...
            if data != -1:
                ...
                🎲 Get ready for the quiz <b>'{quiz_data[6]}</b>'

🖊 <b>{size}</b> question
⏱ <b>{quiz_data[5]}</b> seconds per question
🏁 Press the button below when you are ready.
Send /stop to stop it.""", parse_mode='html')
                y = bot.send_message(msg_id, "The quiz will start in <b>10</b> seconds.", parse_mode="html")
                for i in timer():
                    if type(i) == int:
                        bot.edit_message_text(f"The quiz will start in <b>{i}</b> seconds.", msg_id, y.id, parse_mode="html")
                    else:
                        bot.edit_message_text(f"<b>{i}</b>", msg_id, y.id,parse_mode="html")
        ...
        if user_data['time'] != 0:
            ...
            if (extension_.lower() == "csv") or extension_.lower() == "xlsx":
                ...
                if key != -1:
                    ...
                    text2 = f"""<b>Organized by</b>: {user_data['username']}
<b>Time for each question</b>: {user_data['time']} Seconds
<b>Key</b>: {key}
<b>Description</b>: {user_data['description']}"""
    ...
    try:
        ...
        for i in res:
            if count == 1:
                ...
                _ = f"<b>{count}</b>. {i.get('fullname')}-<b>{i.get('score')}</b> ({i.get('time_taken')})"
            text.append(_)
            count += 1
            if count > limit:
                break
...
@app.route("/")
def index():
    return "Welcome To DishaTestBoard."

if __name__=="__main__":
    try:
        serve(app=app, host='0.0.0.0', port=8443)
    except Exception as E:
        print(E)
